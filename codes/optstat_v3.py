...
if __name__ == '__main__':
    ...
    print 'Initializing model...'
    # initialize the model using the results of the individual noise analyses
    noisedir = '../data/noisefiles/' + dataset + '/'
    h5file = '../data/simulated_data/' + dataset + '/sim.hdf5'
    
    model = init_os_individual(dataset, h5file, psrlist, 30, noisedir,
                               noVaryNoise=True,
                               incJitterEquad=False, incEquad=False,
                               outputfile=outputdir+'/init_os.dat')

    # now fix all the red noise parameters and the GWB parameters
    # to the values from the maximum likelihood sample from the common noise analysis
    print 'Computing the fixed noise OS using the common noise analysis...'
    ...
    preal = chain[index, :-4]
    preal = np.concatenate((preal, np.array([4.33])))
    xi, rho, sig, Opt, Sig = model.opt_stat_mark9(preal, fixWhite=True)

    f = open(outputdir+'/init_os.dat', 'w')
    f.write('Opt  {0}\n'.format(Opt))
    f.write('Sig  {0}\n'.format(Sig))
    f.write('SNR  {0}\n'.format(Opt/Sig))
    f.close()
