...
if git diff --name-only $TRAVIS_COMMIT_RANGE | grep 'manuscript/'
then
  ...
  conda info -a
  conda create --yes -n manuscript
  source activate manuscript
  conda install -c conda-forge -c pkgw-forge tectonic

  # Build the paper using tectonic
  cd manuscript
  tectonic manuscript.tex --print

  # Force push the paper to GitHub
  cd $TRAVIS_BUILD_DIR
  git checkout --orphan $TRAVIS_BRANCH-pdf
  git rm -rf .
  git add -f manuscript/manuscript.pdf
