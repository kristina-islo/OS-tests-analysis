...
for a sample dataset with $\Agw=5\times10^{-15}$. For this particular realization, 
the noise-marginalized optimal statistic 
using the true ORF found $\hat{A}^2=( \pm ) \times 10^{-29}$ 
and $\rho = \pm $. 
None of the 725 scrambled ORFs resulted in a mean SNR greater than 4.8 ($p < 0.0014$).
\begin{figure}[htb]
	...
	\caption{Comparison between the noise-marginalized mean optimal statistic and mean SNR 
			...
			containing an injected GWB with $\Agw = 5\times10^{-15}$. 
			The true ORF gave a mean SNR of 4.8. 
			None of the 725 scrambled ORFs gave a mean SNR this large ($p < 0.0014$).}
	\label{fig:skyscrambles_dataset_sample}
\end{figure}

In Fig.~\ref{fig:skyscrambles_stats}, we plot the distribution of $p$-values of the 725 sky scrambles 
for 200 realizations of the GWB. 
