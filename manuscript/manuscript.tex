...
optimal statistic for a single realization of the stochastic background with $\Agw=$. 
...
to the mean $\hat{A}^2$ for 200 different realizations of a GWB with $\Agw = 5\times10^{-15}$. 
...
$\hat{A}^2 = 2.4\times10^{-29}$ and a mean SNR of 4.1.
\begin{figure}[ht]
	...
	\caption{Optimal statistic and SNR for 200 simulated datasets 
			...
			We compare the values found by fixing the red noise 
			...
			while using the values from a common noise analysis more accurately estimates $\hat{A}^2$.}
	\label{fig:os_datasetstats}
\end{figure}

The fixed-noise and noise-marginalized analyses gave the same results 
...
which the noise-marginalized analysis gives a mean value of $\hat{A}^2 = 1.1\times10^{-30}$ and a mean SNR of 2.4.  
The difference between the fixed-noise and noise-marginalized analyses using the common noise values 
is very small compared to the large spread in $\hat{A}^2$ and the SNR over different realizations of the GWB.
\begin{figure}[ht]
	\includegraphics[width=0.9\columnwidth]{plots/optstat_A1e-15.pdf}
	\caption{Optimal statistic and SNR for 100 simulated datasets 
			containing an injected GWB. 
			The dashed vertical line indicates the injected value, $\hat{A}^2 = 10^{-30}$. 
			We compare the values found by fixing the red noise 
			based on the results from individual and common noise analyses 
			(blue and orange lines, respectively) 
			to the mean optimal statistic and mean SNR found by marginalizing over 
...
The optimal statistic is a computationally inexpensive 
method for assessing the significance of an isotropic stochastic GWB in PTA data. 
The optimal statistic specifically looks for the spatial correlations characteristic 
of a GWB. These analyses are computationally expensive using a full Bayesian approach, 
but with the optimal statistic the significance of different spatial correlations 
can be computed very quickly. 
This makes the optimal statistic particularly well-suited for sky scramble analyses, 
where the ORF is scrambled many times to simulate randomizing the pulsars' sky locations.
