...
This is done by computing the pulsar timing residuals, 
...
particularly when searching for evidence of the spatial correlations characteristic of a 
stochastic GWB -- ``the smoking gun'' of a stochastic GWB.
...
For the noise-marginalized optimal statistic, we draw the red noise parameters from 
the common red noise posteriors in order to average the optimal statistic over 
the individual pulsars' red noise. 
For comparison, we also compute the fixed-noise optimal statistic in two ways -- 
first, by fixing the red noise parameters at the maximum-likelihood values from individual pulsar analyses; 
second, by fixing the red noise parameters at the maximum-likelihood values from a common analysis.
